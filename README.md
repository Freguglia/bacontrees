...
bt$setContextPriorWeights(function(node) -node$getDepth() / 3)
...
| Method | Description |
|----|----|
| `$setAlpha(alpha)` | Set Dirichlet prior parameter |
| `$setContextPriorWeights(fn)` | Assign per-node prior log-weights |
